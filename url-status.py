...
async def async_parse_sitemap(url: str) -> List[str]:
    """
    Asynchronously fetches a sitemap URL and parses <loc> tags (using ElementTree).
    """
    out = []
    try:
...
async def async_parse_sitemap_advertools(url: str) -> List[str]:
    """
    Offloads advertools' sitemap parsing to a thread to avoid blocking the event loop.
    """
    ...
    try:
        ...
        logging.error(f"Error using advertools for sitemap parsing on {url}: {e}")
        return []

async def process_sitemaps(sitemap_urls: List[str], show_partial_callback=None, use_advertools: bool = False) -> List[str]:
    """
    Processes multiple sitemap URLs concurrently, using either the default parser or advertools.
    """
    all_urls = []
    tasks = []
    for sm in sitemap_urls:
        if use_advertools:
            tasks.append(async_parse_sitemap_advertools(sm))
        else:
            tasks.append(async_parse_sitemap(sm))
    ...
    if mode == "Dynamic Frontier":
        ...
        if include_sitemaps:
            ...
            if sitemaps_text.strip():
                ...
                with st.expander("Discovered Sitemap URLs", expanded=True):
                    ...
                    sitemap_urls = loop.run_until_complete(process_sitemaps(raw_sitemaps, show_partial_callback=show_partial_sitemap, use_advertools=use_advertools))
                    loop.close()
        ...
        if st.button("Fetch & Crawl Sitemaps"):
            ...
            with st.expander("Discovered Sitemap URLs", expanded=True):
                ...
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                all_sitemap_urls = loop.run_until_complete(process_sitemaps(lines, show_partial_callback=show_partial_sitemap, use_advertools=use_advertools))
                loop.close()
